...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    selected_paragarphs = [ i for i in paragraphs if select(i)]
    return '' if k >= len(selected_paragarphs) else selected_paragarphs[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def find_topic(s):
        words = split(lower(remove_punctuation(s)))
        for i in words:
            if(i in topic):
                return True
        return False
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    count = 0 #record the correct words
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    min_diff = limit
    min_words = typed_word
    for i in valid_words:
        if(diff_function(typed_word, i, limit) < min_diff):
            min_diff = diff_function(typed_word, i, limit)
            min_words = i
        elif(diff_function(typed_word, i, limit) == min_diff and min_words == typed_word):
            min_diff = diff_function(typed_word, i, limit)
            min_words = i
    return min_words
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    accurency = count / len(prompt)
    d = {'id' : user_id,'progress' : accurency}
    upload(d)
    return accurency
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    [[6, 3, 6, 2], [10, 6, 1, 2]]
    """
    time = [[i[j+1] - i[j] for j in range(len(i) - 1)] for i in times_per_player]
    # BEGIN PROBLEM 9
    ...
    for i in word_indices:
        for j in player_indices:
            ...
            if(time(match, j, i) < min_time):
                min = j
                min_time = time(match, j, i)
        answer[min].append(word_at(match,i))
    return answer
