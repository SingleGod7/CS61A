...
def pingpong(n):
    ...
    True
    """
    def helper(answer,t,step):
        if(t == n):
            return answer 
        if(t % 8 == 0 or num_eights(t)):
            return helper(answer - step, t + 1, -step)
        else:
            return helper(answer + step, t + 1, step)
...
def missing_digits(n):
    ...
    def helper(cur,n):
        if(n == 0):
            ...
            return helper(n % 10, n // 10) + cur - (n % 10) - 1
    return helper(n % 10, n // 10)
...
def count_coins(change):
    ...
    def helper(change, money):
        ...
        if(money == None):
            return 0
        return helper(change - money, money) + helper(change, ascending_coin(money))
